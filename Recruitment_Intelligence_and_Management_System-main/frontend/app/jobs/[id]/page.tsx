...
export default function PublicJobDetailPage() {
    ...
    const handleResendOtp = async () => {
        ...
        try {
            await register(authEmail, authPassword, authName)
            setAuthError(null)
            alert('A new OTP has been sent to ' + authEmail + '. Please also check your Spam/Junk folder.')
        } catch (err: any) {
            // If user already exists unverified, re-registration will resend the OTP
            if (err.message?.toLowerCase().includes('already') || err.message?.toLowerCase().includes('registered')) {
                alert('A new OTP has been sent to ' + authEmail + '. Please also check your Spam/Junk folder.')
            } else {
                setAuthError('Failed to resend OTP: ' + err.message)
            }
        } finally {
            setAuthLoading(false)
        }
    ...
    const handleTabSave = async () => {
        ...
        try {
            ...
            if (idx < TABS.length - 1) {
                ...
                // All tabs done → submit application
                    ...
                    <form onSubmit={handleVerifyOtp} className="space-y-8 text-left">
                        ...
                        <div className="p-3 bg-amber-50 border border-amber-100 rounded-lg">
                            <p className="text-sm text-amber-800 font-medium text-center">
                                📧 Please also check your <strong>Spam / Junk</strong> folder if you don't see the email in your inbox.
                            </p>
                        </div>
                        ...
                        <p className="text-center text-sm text-gray-400 font-medium">
                            ...
                            <button
                                ...
                                disabled={authLoading}
                                className="text-purple-700 font-bold hover:underline ml-1 disabled:opacity-50"
                            >
                ...
                <form onSubmit={handleQuickSubmit}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                            {/* DOB */}
                            <div>
                                <label className="block text-sm text-purple-700 mb-1.5 font-semibold">Date of Birth</label>
                                <div className="relative">
                                    <input type="date" value={quick.dob} onChange={e => setQuick({...quick, dob: e.target.value})}
                                        className="w-full border-b border-gray-300 bg-transparent py-2.5 pr-8 text-base focus:outline-none focus:border-purple-600" required />
                                </div>
                            </div>
                            {/* Interview City */}
                            <div>
                                <label className="block text-sm text-purple-700 mb-1.5 font-semibold">Preferred Interview City</label>
                                <input value={quick.interviewCity} onChange={e => setQuick({...quick, interviewCity: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-purple-600" />
                            </div>
                            {/* Work Location */}
                            <div>
                                <label className="block text-sm text-purple-700 mb-1.5 font-semibold">Preferred Work Location</label>
                                <div className="relative">
                                    <select value={quick.workLocation} onChange={e => setQuick({...quick, workLocation: e.target.value})}
                                        className="w-full border-b border-gray-300 bg-transparent py-2.5 pr-8 text-base focus:outline-none focus:border-purple-600 appearance-none">
                                        <option value=""></option>
                            ...
                            {/* Employer */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Current/Previous Employer</label>
                                <input value={quick.employer} onChange={e => setQuick({...quick, employer: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Total Exp */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Total Experience in Years</label>
                                <input type="number" min="0" value={quick.totalExp} onChange={e => setQuick({...quick, totalExp: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Notice Period */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Notice Period (days)</label>
                                <input value={quick.noticePeriod} onChange={e => setQuick({...quick, noticePeriod: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* CTC */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Current CTC (In Lakhs/INR)</label>
                                <input value={quick.currentCtc} onChange={e => setQuick({...quick, currentCtc: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Diploma */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Diploma Percentage (if applicable)</label>
                                <input value={quick.diplomaPercent} onChange={e => setQuick({...quick, diplomaPercent: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Graduation */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Graduation Percentage</label>
                                <input value={quick.graduationPercent} onChange={e => setQuick({...quick, graduationPercent: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Post Grad */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Post-Graduation Percentage (if applicable)</label>
                                <input value={quick.postGradPercent} onChange={e => setQuick({...quick, postGradPercent: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600" />
                            </div>
                            {/* Mobile */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Mobile No.</label>
                                <div className="flex items-center gap-2">
                                    <span className="text-base font-medium text-gray-500">+</span>
                                    <input value={quick.countryCode} onChange={e => setQuick({...quick, countryCode: e.target.value})}
                                        className="w-16 border-b border-gray-300 bg-transparent py-2.5 text-base text-center focus:outline-none focus:border-gray-600" placeholder="91" />
                                    <span className="text-base text-gray-400">-</span>
                                    <input value={quick.mobile} onChange={e => setQuick({...quick, mobile: e.target.value})}
                            ...
                            {/* PAN */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Pan Number</label>
                                <input value={quick.panNumber} onChange={e => setQuick({...quick, panNumber: e.target.value.toUpperCase()})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600 uppercase font-bold text-gray-800" placeholder="ABCDE1234F" />
                            </div>
                            {/* Skills (full width) */}
                            <div className="sm:col-span-2">
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Skills</label>
                                <input value={quick.skills} onChange={e => setQuick({...quick, skills: e.target.value})}
                                    className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600 font-medium" placeholder="React, Node.js, Python..." />
                            </div>
                            {/* Gender */}
                            <div>
                                <label className="block text-sm text-gray-500 mb-1.5 font-semibold">Gender</label>
                                <div className="relative">
                                    <select value={quick.gender} onChange={e => setQuick({...quick, gender: e.target.value})}
                                        className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600 appearance-none pr-6">
                                        <option value=""></option>
                            ...
                            <div className="flex flex-col lg:flex-row gap-8">
                                ...
                                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input value={personal.mobile} onChange={e => setPersonal({...personal, mobile: e.target.value})}
                                                className="flex-1 border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600 font-semibold" placeholder="9876543210" />
                                        </div>
                                    </div>
                                    {/* Gender */}
                                    <div>
                                    ...
                                    <div className="sm:col-span-2">
                                        ...
                                        <input value={personal.panNumber} onChange={e => setPersonal({...personal, panNumber: e.target.value.toUpperCase()})}
                                            className="w-full border-b border-gray-300 bg-transparent py-2.5 text-base focus:outline-none focus:border-gray-600 uppercase font-bold tracking-widest text-gray-800" placeholder="ABCDE1234F" />
                                    </div>
                                </div>
                            </div>
                        </div>
