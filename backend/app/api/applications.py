...
async def apply_for_job(
    ...
    # 1. Notify candidate immediately of receipt with AI-generated professional message
    background_tasks.add_task(send_application_received_email, candidate_email, job.title, job.company_name)
...
def _ensure_interview_record(application: Application, db) -> str:
    ...
    if not existing_interview:
        ...
        unique_test_id = "TEST-" + str(uuid.uuid4().hex)[:8].upper()
        ...
        if not existing_interview.test_id:
            existing_interview.test_id = "TEST-" + str(uuid.uuid4().hex)[:8].upper()
