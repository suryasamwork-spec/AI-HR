...
import { Briefcase, Search, MapPin, ArrowDown, ChevronRight, Target, Users, Code, Building, Cpu, Globe, Rocket, Shield, BookOpen, Building2, Award, Hexagon, HardHat, Code2, Zap, Layers, BarChart3, LineChart, Star, TrendingUp, Workflow, CheckCircle2, CheckCircle, BrainCircuit, ArrowUpRight, Check, ArrowRight, MousePointerClick, Play, Database, Server, Smartphone, Layout, Blocks, Lock, X } from 'lucide-react';
import caldimLogo from '@/assets/caldim-logo-new.png';
import CareersFooter from '@/modules/careers/components/CareersFooter';
...
const RIMS_URL = import.meta.env.VITE_RIMS_FRONTEND_URL || '/rims';
  ...
  const perks = [
    {
      ...
      desc: 'Work in world-class collaboration hubs designed for innovation and deep focus.',
      icon: <Building2 className="text-blue-400 mb-4" size={32} />
    },
    {
      title: 'Continuous Growth',
      desc: 'Sponsored certifications, dedicated learning hours, and global tech conferences.',
      icon: <BookOpen className="text-purple-400 mb-4" size={32} />
    },
    {
      title: 'Health & Wellness',
      desc: 'Comprehensive premium health coverage, wellness days, and ergonomic setups.',
      icon: <Shield className="text-green-400 mb-4" size={32} />
    },
    {
      title: 'Performance Rewards',
      desc: 'Merit-based bonuses, stock options, and accelerated internal promotions.',
      icon: <Award className="text-yellow-400 mb-4" size={32} />
    }
  ];

  const images = [
    { src: '/images/life-1.jpg', label: 'Collaborative Architecture'},
    { src: '/images/life-2.jpg', label: 'Engineering Hackathons'},
    { src: '/images/life-3.jpg', label: 'Mentorship & Growth'},
  ];

  return (
    <section id="life" className="py-32 bg-[#0f172a] relative overflow-hidden border-t border-white/5">
      {/* Background Atmosphere */}
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-blue-500/5 blur-[150px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <Reveal className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-6xl font-black text-white mb-6">
            Build the Future, <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">Experience the Best.</span>
          </h2>
          <p className="text-white/40 max-w-2xl mx-auto text-lg">
            We believe that to build world-class engineering solutions, you need a world-class environment. Caldim invests heavily in the people who push the boundaries of what is possible.
          </p>
        </Reveal>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
          {perks.map((perk, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-8 hover:bg-white/[0.07] hover:border-blue-500/30 transition-all group h-full">
                {perk.icon}
                <h3 className="text-white font-bold text-xl mb-3 group-hover:text-blue-300 transition-colors">{perk.title}</h3>
        ...
        {/* Visual Showcase (Modified Bento) */}
        <Reveal>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-auto lg:h-[600px]">
            {/* Main Showcase Image (Left) */}
            <div className="lg:col-span-8 relative rounded-3xl overflow-hidden group min-h-[300px] h-full">
              <img src={images[0].src} alt={images[0].label} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/90 via-[#0f172a]/20 to-transparent" />
              <div className="absolute inset-0 p-8 flex flex-col justify-end">
                <span className="text-blue-400 text-xs font-bold uppercase tracking-widest mb-2 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0">Global Campuses</span>
                <h3 className="text-white text-3xl font-bold leading-tight drop-shadow-lg">{images[0].label}</h3>
              </div>
            </div>

            {/* Side Column Stack */}
            <div className="lg:col-span-4 grid grid-rows-2 gap-4 h-full min-h-[400px] lg:min-h-0">
              {/* Top Right */}
              <div className="relative rounded-3xl overflow-hidden group h-full">
                <img src={images[1].src} alt={images[1].label} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/90 to-transparent" />
                <div className="absolute inset-0 p-6 flex flex-col justify-end">
                  <h3 className="text-white text-xl font-bold drop-shadow-md">{images[1].label}</h3>
                </div>
              </div>

              {/* Bottom Right */}
              <div className="relative rounded-3xl overflow-hidden group h-full">
                <img src={images[2].src} alt={images[2].label} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/90 to-transparent" />
                <div className="absolute inset-0 p-6 flex flex-col justify-end">
                  <h3 className="text-white text-xl font-bold drop-shadow-md">{images[2].label}</h3>
                </div>
              </div>
            </div>
          </div>
  ...
  const domains = [
    {
      ...
      tech: [
        { 
          ...
          icon: Layers,
          info: 'We utilize Tekla Structures to construct highly detailed, constructible 3D models for steel and concrete structures. This allows us to ensure clash-free site erection and exact material bills.',
          highlights: ['LOD 400 & 500 Modeling', 'Automated Fabrication Drawings', 'Clash Detection & Resolution']
        },
        { 
          name: 'SDS/2 Detailing', 
          icon: Cpu,
          info: 'Our team leverages SDS/2 for intelligent connection design that inherently understands fabrication and erection constraints, automating complex structural joinery.',
          highlights: ['Intelligent Connection Design', 'Erection-First Principles', 'CNC File Generation']
        },
        { 
          name: 'Rebar Connectivity', 
          icon: BarChart3,
          info: 'We provide specialized and heavily detailed reinforcement modeling. This includes intricate rebar configurations for heavy foundations, transfer slabs, and seismic designs.',
          highlights: ['3D Rebar Detailing', 'Bar Bending Schedules (BBS)', 'Constructability Reviews']
        },
        { 
          name: 'BIM Integration', 
          icon: Globe,
          info: 'Our Building Information Modeling (BIM) workflow unifies architectural, structural, and MEP engineering data into a single source of truth for seamless project lifecycle management.',
          highlights: ['Multi-Disciplinary Coordination', 'Navisworks Clash Management', 'IFC Standard Compliance']
        },
      ],
    ...
    },
    {
      ...
      tech: [
        { 
          name: 'End-to-End Web', 
          icon: Code2,
          info: 'We architect and build scalable, secure, and blazing-fast web platforms using the modern React/Next.js ecosystem and high-performance backend microservices.',
          highlights: ['React, Next.js, & Vue Ecosystems', 'API-Driven Architecture', 'Enterprise-Grade Security']
        },
        { 
          name: 'Mobile (iOS/Android)', 
          icon: Zap,
          info: 'Creating seamless, native-feeling mobile experiences that connect users to your core services on the go, utilizing React Native and Flutter for high-quality cross-platform delivery.',
          highlights: ['Cross-Platform Development', 'Native Performance Optimization', 'Offline-First Architectures']
        },
        { 
          name: 'UI/UX & Product', 
          icon: Layers,
          info: 'Designing intuitive, breathtaking digital interfaces that prioritize user experience and conversion. We blend aesthetic brilliance with deep user psychology.',
          highlights: ['High-Fidelity Prototyping', 'Design Systems & Component Libraries', 'User Research & Journey Mapping']
        },
        { 
          name: 'Cloud & DevOps', 
          icon: Globe,
          info: 'Automating the deployment lifecycle and architecting cloud infrastructure that scales automatically, ensuring 99.99% uptime and uncompromised data integrity.',
          highlights: ['AWS, Azure, & GCP Specialization', 'CI/CD Pipeline Automation', 'Docker & Kubernetes Orchestration']
        },
      ],
        ...
        {selectedTech && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" style={{ pointerEvents: 'auto' }}>
            <motion.div
              ...
              className="absolute inset-0 bg-[#06101f]/80 backdrop-blur-md cursor-pointer"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className={`relative z-10 w-full max-w-2xl bg-[#0f172a] border ${selectedTech.theme === 'orange' ? 'border-orange-500/30 shadow-orange-500/20' : 'border-blue-500/30 shadow-blue-500/20'} rounded-[32px] p-8 md:p-12 shadow-2xl overflow-hidden`}
            >
              {/* Decorative Corner Glow inside Modal */}
              <div className={`absolute -top-32 -right-32 w-64 h-64 blur-[80px] rounded-full opacity-20 pointer-events-none ${selectedTech.theme === 'orange' ? 'bg-orange-500' : 'bg-blue-500'}`} />

              <button
                onClick={() => setSelectedTech(null)}
                className="absolute top-6 right-6 text-white/50 hover:text-white transition-colors p-2 rounded-full hover:bg-white/10 z-20"
              >
                <X size={24} />
              </button>
              
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 relative z-10 ${selectedTech.theme === 'orange' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>
                 <selectedTech.icon size={32} />
              </div>

              <div className="relative z-10">
                <div className={`text-[10px] font-black uppercase tracking-[0.2em] mb-3 inline-block px-3 py-1 rounded-full border ${selectedTech.theme === 'orange' ? 'border-orange-500/30 text-orange-400 bg-orange-500/10' : 'border-blue-500/30 text-blue-400 bg-blue-500/10'}`}>
                  {selectedTech.domainTag} Specialization
                </div>
                <h3 className="text-3xl md:text-4xl font-black text-white mb-6 leading-tight">
                  {selectedTech.name}
                </h3>
                
                <p className="text-white/60 text-base md:text-lg leading-relaxed mb-10 pb-10 border-b border-white/10">
                  {selectedTech.info}
                </p>

                <div className="space-y-5">
                  <h4 className="text-white font-bold text-xs tracking-widest uppercase mb-4 opacity-50">Core Capabilities</h4>
                  <div className="grid sm:grid-cols-1 gap-4">
                    {selectedTech.highlights.map((h, idx) => (
                      <div key={idx} className="flex items-center gap-4 bg-white/[0.03] p-4 rounded-xl border border-white/5">
                        <div className={`w-2 h-2 rounded-full ${selectedTech.theme === 'orange' ? 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.8)]' : 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]'}`} />
                        <span className="text-white/80 font-medium text-sm">{h}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6">
                {domains[active].tech.map((t, ti) => (
                  <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    key={ti} 
                    onClick={() => setSelectedTech({ ...t, theme: domains[active].accent, domainTag: domains[active].title })}
                    className={`relative flex text-left items-center justify-between gap-4 bg-white/[0.05] border border-white/10 rounded-2xl p-4 group hover:shadow-2xl transition-all duration-300 w-full overflow-hidden ${active === 0 ? 'hover:border-orange-500/50 hover:shadow-orange-500/10' : 'hover:border-blue-500/50 hover:shadow-blue-500/10'}`}
                  >
                    {/* Animated gradient background on hover */}
                    <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-300 bg-gradient-to-r ${active === 0 ? 'from-orange-500 to-transparent' : 'from-blue-500 to-transparent'}`} />
                    
                    <div className="flex items-center gap-4 relative z-10 w-full">
                      <div className={`w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center transition-all shadow-inner ${active === 0 ? 'bg-orange-500/10 text-orange-400 group-hover:bg-orange-500 group-hover:text-white' : 'bg-blue-500/10 text-blue-400 group-hover:bg-blue-500 group-hover:text-white'}`}>
                        <t.icon size={20} />
                      </div>
                      <div className="flex-1">
                        <span className="block text-sm font-black text-white group-hover:text-white transition-colors">{t.name}</span>
                        <span className={`text-[9px] font-black uppercase tracking-widest mt-1 block transition-all ${active === 0 ? 'text-orange-500/70 group-hover:text-orange-300' : 'text-blue-500/70 group-hover:text-blue-300'}`}>
                           Click to Explore
                        </span>
                      </div>
                    </div>

                    <div className={`relative z-10 flex-shrink-0 w-8 h-8 rounded-full border flex items-center justify-center opacity-80 group-hover:opacity-100 transition-all ${active === 0 ? 'border-orange-500/40 text-orange-400 group-hover:border-orange-500 group-hover:bg-orange-500/20 animate-pulse group-hover:animate-none' : 'border-blue-500/40 text-blue-400 group-hover:border-blue-500 group-hover:bg-blue-500/20 animate-pulse group-hover:animate-none'}`}>
...
const JobsSection = () => {
  ...
  const groupedJobs = jobs.reduce((acc, job) => {
    const domainStr = (job.domain || '').toLowerCase();
    const titleStr = (job.title || '').toLowerCase();
    
    const isCivil = domainStr.includes('civil') || domainStr.includes('structur') || domainStr.includes('bim') || domainStr.includes('core') || titleStr.includes('civil') || titleStr.includes('structural') || titleStr.includes('bim') || titleStr.includes('detailer');
    
    const isHR = domainStr.includes('hr') || domainStr.includes('human') || domainStr.includes('talent') || domainStr.includes('operation') || domainStr.includes('finance') || domainStr.includes('admin') || titleStr.includes('hr ') || titleStr.includes('recruiter');
    
    const isSoftware = domainStr.includes('software') || domainStr.includes('tech') || domainStr.includes('digital') || domainStr.includes('it') || titleStr.includes('software') || titleStr.includes('developer') || titleStr.includes('ai ') || titleStr.includes('ai analyst') || titleStr.includes('analyst') || titleStr.includes('data') || titleStr.includes('backend') || titleStr.includes('frontend') || titleStr.includes('web');

    if (isCivil) {
      acc.Civil.push(job);
    } else if (isSoftware) {
      acc.Software.push(job);
    } else if (isHR) {
      acc.HR.push(job);
    } else {
      acc.Other.push(job);
    ...
    return acc;
  }, { Software: [], Civil: [], HR: [], Other: [] });

  const categories = [
    { id: 'Software', label: 'Software & Digital', icon: Code2, color: 'blue' },
    { id: 'Civil', label: 'Civil & Structural', icon: HardHat, color: 'orange' },
    { id: 'HR', label: 'HR & Operations', icon: Users, color: 'purple' },
    { id: 'Other', label: 'Other Opportunities', icon: Briefcase, color: 'emerald' }
  ];
  // Always display all valid company domains
  const availableCategories = categories;

  // Auto-select first category once loaded
  useEffect(() => {
    if (!loading && availableCategories.length > 0 && !activeCategory) {
      setActiveCategory(availableCategories[0].id);
    }
  }, [loading, jobs, availableCategories, activeCategory]);

  const renderJobCard = (job, i, catInfo) => {
    const Icon = catInfo.icon;
    
    // Explicit static tailwind classes for theme styling
    const themes = {
      blue: { text: 'text-blue-400', bg: 'bg-blue-500/10', borderHover: 'hover:border-blue-500/40', glow: 'bg-blue-500/10', cardHoverText: 'group-hover:text-blue-300', btnHoverBg: 'group-hover/btn:bg-blue-500', btnHoverBorder: 'group-hover/btn:border-blue-400', btnHoverText: 'group-hover/btn:text-white' },
      orange: { text: 'text-orange-400', bg: 'bg-orange-500/10', borderHover: 'hover:border-orange-500/40', glow: 'bg-orange-500/10', cardHoverText: 'group-hover:text-orange-300', btnHoverBg: 'group-hover/btn:bg-orange-500', btnHoverBorder: 'group-hover/btn:border-orange-400', btnHoverText: 'group-hover/btn:text-white' },
      purple: { text: 'text-purple-400', bg: 'bg-purple-500/10', borderHover: 'hover:border-purple-500/40', glow: 'bg-purple-500/10', cardHoverText: 'group-hover:text-purple-300', btnHoverBg: 'group-hover/btn:bg-purple-500', btnHoverBorder: 'group-hover/btn:border-purple-400', btnHoverText: 'group-hover/btn:text-white' },
      emerald: { text: 'text-emerald-400', bg: 'bg-emerald-500/10', borderHover: 'hover:border-emerald-500/40', glow: 'bg-emerald-500/10', cardHoverText: 'group-hover:text-emerald-300', btnHoverBg: 'group-hover/btn:bg-emerald-500', btnHoverBorder: 'group-hover/btn:border-emerald-400', btnHoverText: 'group-hover/btn:text-white' },
    };
    const t = themes[catInfo.color];

    return (
      <Reveal key={job.id || i} delay={i * 0.08}>
        <a href={`${RIMS_URL}/jobs/${job.id}`} className={`group relative p-8 rounded-[32px] bg-[#0f172a] border border-white/8 ${t.borderHover} hover:bg-[#162032] transition-all duration-500 flex flex-col h-full overflow-hidden`}>
          <div className={`absolute top-0 right-0 w-32 h-32 ${t.glow} blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700`} />
          
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex justify-between items-start mb-8">
              <div className={`p-3 rounded-2xl ${t.bg} ${t.text}`}>
                <Icon size={24} />
              </div>
              <span className="text-white/30 text-[10px] font-bold uppercase tracking-widest pt-2">{job.job_type}</span>
            </div>

            <h3 className={`text-2xl font-bold text-white mb-4 transition-colors leading-tight uppercase ${t.cardHoverText}`}>{job.title}</h3>
            ...
            <div className="mt-auto pt-8 border-t border-white/[0.04] flex items-center justify-between">
              <div className="flex items-center gap-2 text-white/50 text-xs">
                <MapPin size={14} className={`${t.text} opacity-70`} />
              ...
              <div className="flex items-center gap-2 text-white font-bold text-xs uppercase tracking-widest group/btn">
                ...
                <div className={`w-8 h-8 rounded-full bg-white/8 border border-white/15 flex items-center justify-center transition-all duration-300 group-hover/btn:scale-110 ${t.btnHoverBg} ${t.btnHoverBorder}`}>
                  <ArrowRight size={14} className={`${t.btnHoverText} text-white/50`} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            {availableCategories.length > 0 ? (
              <div className="space-y-10">
                ...
                <div className="flex flex-wrap gap-4 pb-4 border-b border-white/10">
                  {availableCategories.map((cat) => {
                    ...
                    const activeColor = themeColors[cat.color];
                    
                    return (
                      <button
                        key={cat.id}
                        onClick={() => setActiveCategory(cat.id)}
                        className={`flex items-center gap-3 px-5 py-3 rounded-2xl border transition-all duration-300 font-bold text-sm ${isActive ? activeColor + ' ring-1 ring-white/10 scale-100 shadow-xl' : 'text-white/40 bg-white/5 border-transparent hover:bg-white/10 hover:text-white'}`}
                      >
                        <cat.icon size={18} className={isActive ? '' : 'opacity-60'} />
                        {cat.label}
                        <span className={`ml-2 px-2 py-0.5 rounded-full text-[10px] ${isActive ? 'bg-white/20 text-current' : 'bg-white/10 text-white/40'}`}>
                          {groupedJobs[cat.id].length}
                        </span>
                      </button>
                    );
                  })}
                ...
                {/* Job Cards Grid for Active Category */}
                <AnimatePresence mode="wait">
                   <motion.div 
                     key={activeCategory}
                     ...
                     className="w-full"
                   >
                     {activeCategory && groupedJobs[activeCategory].length > 0 ? (
                       <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                         {groupedJobs[activeCategory].map((job, i) => 
                            renderJobCard(job, i, categories.find(c => c.id === activeCategory))
                         )}
                       </div>
                     ) : (
                       <div className="py-20 text-center border border-white/5 rounded-[32px] bg-[#0f172a]">
                         ...
                         <p className="text-white/40 text-sm">We currently don't have open positions in this domain.<br/>Check back later or explore other categories.</p>
                       </div>
                     )}
                   </motion.div>
                </AnimatePresence>
              ...
              <div className="text-center py-12">
                <p className="text-white/40">No current openings available. Check back soon!</p>
              </div>
            )}
          </div>
        )}
